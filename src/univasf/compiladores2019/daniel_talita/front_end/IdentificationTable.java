...
//Construtor
    ...
    //Adiciona uma entrada na tabela associando o identificador com o atributo 
        i = 0;
        while ((i <= last) && (!identificador.get(i).equals(var.getNome()))) i++;
        if (i > last) {
            last++;
            identificador.add(last, var);
        }
        else {
            throw new ContextualError("Erro! A variavel" +var.getNome()+ " está sendo declarada duas vezes"); //Devemos retornar a linha e a colunas aqui? Se sim. Como? Chamando Scanner?
        }
    }
